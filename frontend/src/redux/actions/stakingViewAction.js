...
function stakingViewAct(account) {
    return async (dispatch) => {
        try {
            if(account !== ''){
                const myNftListApi = await api.post("/getExceptSellOwnedJolamanType", { account });
                const stakingNftApi = await api.post("/block/stakedJolaman", { account });
                const stakingRewardApi = await api.post("/updateReward", { account });
                const getStakingRewardApi = await api.post("/stakers", { account });
